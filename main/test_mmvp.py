...
from metrics.evaluate_mmvp import clip_fwd
import clip
...
def main(config):
    ...
    for expr_name, config_target in SETTINGS["experiments"].items():
        if expr_name == "mmvp_benchmark":
            ...
            args = {"preprocess": model.img_preprocess, "model": model, "tokenize_func": clip.tokenize, "clip_fwd_func": clip_fwd, "benchmark_dir": config_target["dataset_mmvp"]["benchmark_dir"]}
    ...
    # Report
    result_path = os.path.join(out, "test_mmvp_result.json")
...
def run():
    ...
    parser.add_argument("--results_dir", type=str, default="/lpai/output/models/clip-refine/cc3m-openaiclip-vitL14-fixval", help="directory to save the results to")
